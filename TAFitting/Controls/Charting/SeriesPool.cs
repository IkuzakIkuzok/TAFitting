...
/// <summary>
/// Provides a pool for reusing <see cref="Series"/> instances to optimize resource usage and reduce allocations within charting operations.
/// </summary>
/// <remarks>This class is intended for internal use to manage the lifecycle of <see cref="Series"/> objects.
...
internal sealed class SeriesPool
{
    ...
    private readonly Stack<Series> pool = [];

    /// <summary>
    /// Retrieves a reusable <see cref="Series"/> instance from the pool, or creates a new one if the pool is empty.
    /// </summary>
    ...
    /// <returns>A <see cref="Series"/> instance that can be used by the caller. The returned instance may be newly created or previously used.</returns>
    internal Series Rent()
    {
        var series = this.pool.Count > 0
            ? this.pool.Pop()
            : new Series($"Pool-{Interlocked.Increment(ref this.seriesCount)}");

        return series;
    } // internal Series Rent ()

    /// <summary>
    /// Returns a <see cref="Series"/> instance to the pool for reuse after resetting its state.
    /// </summary>
    ...
    /// <param name="series">The <see cref="Series"/> instance to be returned to the pool.</param>
    internal void Return(Series series)
    {
        series.Points.Clear();
